...
namespace KHAS {
    ...
    private:
        ...
        void printFindingTheRootOfANonlinearEquation(double x, size_t count_iterations, std::ostream& out = std::cout) const noexcept;

        std::vector<DB> rungeKuttaMethodInArray() const noexcept;
        void printRungeKuttaMethod(std::ostream& out = std::cout) const noexcept;
        ...
        void printLinearInterpolation(std::ostream& out = std::cout) const noexcept;

        void printIntegralTrapezoidMethod(std::ostream& out = std::cout) const noexcept;
